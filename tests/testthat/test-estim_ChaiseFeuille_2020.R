...
dat_common <- sim_dat_common(timing_treatment = 5:10, as_mdd = TRUE)

mdd_DD <- mdd_DD_simple(dat_common)
mdd_DD_cut <- mdd_DD_simple(dat_common |>subset(Time%in%c(4,5)))
mdd_ES <- mdd_event_study(dat_common)
mdd_CH_24_0 <- mdd_estim_CH(mdd_dat = dat_common)
mdd_CH_24_dynamic <- mdd_estim_CH(mdd_dat = dat_common, effects =5, placebo=3)



## pkg rounds coefficients! mat_res_XX[,1:4] <- round(mat_res_XX[,1:4],5)
test_that("CH with no effects is same as 2-2 (but with tol!!)", {
...
test_that("CH with no effects is same as ES (but with tol!!)", {
...
test_that("CH with effects is same as ES (but with tol!!)", {
  expect_equal(coef(mdd_CH_24_dynamic, type="Effects")[1:5],
...
test_that("ATE is just mean effects?", {
  expect_equal(mean(coef(mdd_CH_24_dynamic, type="Effects")[1:5]),
               coef(mdd_CH_24_dynamic, type="ATE"),
               tolerance=1e-5,
               ignore_attr=TRUE)
  expect_equal(coef(mdd_CH_24_0, type="Effects"),
               coef(mdd_CH_24_0, type="ATE"),
               tolerance=1e-5,
               ignore_attr=TRUE)
})
